  ...
  return (
    ...
    >
      ...
      <div className="mb-8 sm:mb-12">
        <h2 className="text-white font-helvetica text-4xl sm:text-5xl md:text-[52px] leading-tight whitespace-pre-line">
          {title}
        </h2>
        {ghostTitle ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6">
        {/* CTA Card */}
        <div
          className={`lg:col-span-4 ${theme.panel} rounded-[22px] p-6 flex flex-col items-center justify-between min-h-[332px] backdrop-blur-sm relative overflow-hidden`}
        >
          <div
            className="pointer-events-none absolute inset-0 rounded-[22px] bg-gradient-to-br from-white/5 to-transparent"
            aria-hidden="true"
          />
          ...
          <button
            className={`${theme.ctaBtn} transition-colors font-helvetica text-base px-6 py-2 rounded-[22px]`}
          >
            {cta.buttonLabel}
          </button>
        </div>
        ...
        {/* Carousel */}
        <div
          className={`lg:col-span-4 ${theme.panel} rounded-[22px] p-4 sm:p-6 relative backdrop-blur-sm`}
        >
          <Carousel slides={carouselSlides} className="" verticalDots />
          <div
            className="pointer-events-none absolute inset-0 rounded-[22px] bg-gradient-to-b from-white/5 to-transparent mix-blend-overlay"
            aria-hidden="true"
          />
        </div>
        ...
        {/* Checklist Panel */}
        <div
          className={`lg:col-span-4 ${theme.panel} rounded-[22px] p-6 flex flex-col justify-between min-h-[332px] relative overflow-hidden`}
        >
          <div>
            <h3 className="text-white font-helvetica text-xl sm:text-2xl mb-4">
              Your plan
            </h3>
            <ul className="space-y-3">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-white/90">
                  ...
                  <span className="text-sm sm:text-base font-helvetica">
                    {item}
                  </span>
                </li>
              ))}
          ...
          <button
            className={`${theme.ctaBtn} mt-6 w-full transition-colors font-helvetica text-base px-6 py-3 rounded-[12px]`}
          >
            Start your plan
          </button>
          <div
            className="pointer-events-none absolute inset-0 rounded-[22px] bg-gradient-to-tr from-white/5 to-transparent"
            aria-hidden="true"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 relative rounded-[22px] overflow-hidden min-h-[211px] flex items-center justify-center p-6 sm:p-12 shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_10px_30px_rgba(0,0,0,0.35)]">
          ...
          <div className="relative z-10 flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-[#B9B9B9] font-helvetica text-[13px]">
              {testimonial.author}
            </p>
          </div>
        </div>
        ...
        {/* Tags */}
        <div
          className={`lg:col-span-8 ${theme.panel} rounded-[22px] p-4 sm:p-5 min-h-[84px] flex flex-wrap items-center gap-2 sm:gap-3 backdrop-blur-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.06),0_10px_30px_rgba(0,0,0,0.35)]`}
        >
          {tags.map((tag) => (
            <div
              key={tag}
            ...
            >
              <span className="text-white font-helvetica text-sm sm:text-[15px]">
                {tag}
              </span>
            </div>
          ))}
